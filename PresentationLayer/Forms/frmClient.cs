...
namespace PresentationLayer.Forms
{
    public partial class frmClient : Form
    {
        ...
        {
            BL_Client blClient = new BL_Client();
            ...
            string result = blClient.CreateClient(client);
        ...
        private void ibtnEdit_Click(object sender, EventArgs e)
        {
            ...
            string result = new BL_Client().UpdateClient(Convert.ToInt32(txtId.Text), client);
            MessageBox.Show(result);
        ...
        private void dgvList_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvList.Columns[e.ColumnIndex].Name == "Edit")
            {
                if (e.RowIndex >= 0)
                {
                    ...
                    txtName.Text = dgvList.Rows[e.RowIndex].Cells["CName"].Value.ToString();
        ...
        private void SearchBy(string by, string searchString)
        {
            ...
            if (!searchString.Trim().Equals(""))
            {
                ...
                {
                    dgvList.Rows.Clear();
                    clients = clients.Where(o => o.Mail.Contains(searchString));
                }
                else if (by == "Direccion")
                {
                    dgvList.Rows.Clear();
                    clients = clients.Where(o => o.Address.Contains(searchString));
                }
                else if (by == "Telefono")
                {
                    dgvList.Rows.Clear();
                    clients = clients.Where(o => o.Phone.Contains(searchString));
                }
                else if (by == "Estado")
                {
                    dgvList.Rows.Clear();
                    ...
                    clients = clients.Where(o => o.State == state);
                }
            }
            else
            {
            ...
            {
                dgvList.Rows.Add(new object[] { client.Id, client.Document, client.FullName, client.Mail, client.Address, client.Phone,
                    client.State == true ? 1 : 0, client.State == true ? "ACTIVO" : "INACTIVO", "" });
            }
        }
